  ...
  call (ws, id, method, params) {
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.send(JSON.stringify({
        ...
        if (err) {
          reject(`[VSpotWS] Failed to call with web socket error: "${err.message}"`)
        }
      })
    ...
    ws.on('message', async function onMessage (message) {
      ...
      if (data.type === callTypes.REQ) {
        ...
        ws.send(JSON.stringify({
          ...
          if (err) {
            console.error(`[VSpotWS] Failed to response for call request "${id}" with web socket error: ${err.message}`)
          }
        })
      }
    })
